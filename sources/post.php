<?php 
  ...
  if($motty == "send_form" && isset($_POST['send_lienhe']))
  {
    ...
    if ($motty == "send_form1" && isset($_POST['send_lienhe1'])) {
    $admin_email = LAY_email(1); // Assuming LAY_email(1) retrieves admin email
    $htmlbox = file_get_contents("myadmin/htmlbox/1m.html");
    ...
    $footer = "<p><b>" . $thongtin['tenbaiviet_' . $lang] . "</b></p>";
    $footer .= !empty($thongtin['sodienthoai_vi']) ? "<p>" . $glo_lang['so_dien_thoai'] . ": " . $thongtin['sodienthoai_vi'] . "</p>" : "";
    $footer .= !empty($thongtin['email_vi']) ? "<p>" . $glo_lang['email'] . ": " . $thongtin['email_vi'] . "</p>" : "";
    $footer .= !empty($thongtin['diachi_' . $lang]) ? "<p>" . $glo_lang['dia_chi'] . ": " . $thongtin['diachi_' . $lang] . "</p>" : "";

    $file_1 = '';
    $file_2 = '';

    if (!empty($_FILES['inputfile']['name'])) {
        $file_1 = UPLOAD_file("inputfile", "./datafiles/post/", time());
    }

    if (!empty($_FILES['inputfile_1']['name'])) {
        $file_2 = UPLOAD_file("inputfile_1", "./datafiles/post/", time());
    }
    ...
    $message = str_replace(
        array("%footer%", '%logo%', '%thongtinheader%', '%noidung%'),
        array($footer, LOC_char($logo), $thongtinheader, $noidung),
        $htmlbox
    );
    ...
    if (isset($_POST['dowload_file'])) {
        $noidung_download = LAYTEXT_rieng(54); // Assuming LAYTEXT_rieng(54) retrieves specific text for download email
        ob_start();
        GUI_email($s_email, "", $noidung_download['tenbaiviet_' . $lang] . " " . $s_email, $_SERVER['SERVER_NAME'], $noidung_download['noidung_' . $lang], $thongtin);
        ob_end_clean();
    }
