...
function SET_redis($key, $value)
{
    ...
    if ($ob_redis === NULL || $redis_on_off != "on") {
        ...
        if ($cache_file == "on" && $is_myadmin == "off") {
            ...
            if ($thumuc != "") {
                if (!is_dir(__DIR__ . '/../../datafiles/cache')) {
                    mkdir(__DIR__ . '/../../datafiles/cache');
                    chmod(__DIR__ . '/../../datafiles/cache', 0777);
                }
                if (!is_dir(__DIR__.'/../../datafiles/cache/' . $thumuc)) {
                    mkdir(__DIR__.'/../../datafiles/cache/' . $thumuc);
                    chmod(__DIR__.'/../../datafiles/cache/' . $thumuc, 0777);
                }
                if (!empty($thumuc_check[1]) && $thumuc_check[1] != "") {
                    $path = __DIR__.'/../../datafiles/cache/' . $thumuc . "/" . $thumuc_check[1] . ".txt";
                    if (!file_exists($path)) {
