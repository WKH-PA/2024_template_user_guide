...
function LAY_menu1($val = 0, $name = '', $class = '', $kieu = 0, $id_ht = 0, $chude = 'true')
{
    // Fetch the menu items from the database where id_parent = 1
    $chude_arr = DB_fet("*", "#_menu", "`showhi` = '1' AND `id_parent` = '1'", "`catasort` ASC", "", "arr");

    // Build the HTML menu structure directly
    $menu = build_menu_items($chude_arr, $val, 1, $id_ht, $chude);

    // Wrap the menu in a div
    $menu = '<div class="' . $class . '">' . $menu . '</div>';

    return $menu;
}

function build_menu_items($items, $val, $parent_id = 0, $id_ht = 0, $chude = 'true')
{
    $menu = '';

    foreach ($items as $item) {
        if ($item['id_parent'] == $parent_id) {
            $disabled = ($id_ht == $item['id'] && $chude == 'true') ? ' class="disabled"' : '';
            $selected = ($val == $item['id']) ? ' class="selected"' : '';

            $menu .= '<li' . $selected . '><a href="#"' . $disabled . '>' . $item['tenbaiviet_vi'] . '</a>';

            // Recursively build sub-menu items
            $submenu = build_menu_items($items, $val, $item['id'], $id_ht, $chude);
            if ($submenu) {
                $menu .= '<ul>' . $submenu . '</ul>';
            }
    ...
    // Wrap the menu items in a ul if not empty
    if (!empty($menu)) {
        $menu = '<ul>' . $menu . '</ul>';
    }
...
{
    if (!filter_var(trim($email), FILTER_VALIDATE_EMAIL)) return TRUE;
    else return FALSE;
}
...
function GET_menu_2($full_url, $lang, $class_ul = '', $class_li = '', $class_a = '', $id_pr = 1)
{
    ...
    foreach ($tb_menu as $row_1) {
        if ($row_1['id_parent'] != $id_pr) {
            continue; // Bỏ qua các mục không có id_parent bằng $id_pr (menu cấp 2 của $id_pr)
        }

        // Xử lý các mục menu cấp 2
        ...
        $list_mb_2 .= '<li class="is_step_' . $row_1['step'] . ' ' . $class_li . ' hide_' . $row_1['id'] . '">' . MENU_return_link($class_a, $tb_menu, $tb_step, $tb_danhmuc, $row_1['id'], $icon, $icon_hover);

        // Nếu mục menu cấp 2 cũng có menu cấp 3, bạn có thể thêm vòng lặp để xử lý menu cấp 3 tương tự như vậy ở đây
    }
...
function LAY_step1($ids = array(), $limit = 0, $where = "")
{
    if (!empty($ids)) {
        $ids_str = implode(',', $ids);
        $where .= $where != "" ? " AND `id` IN ($ids_str)" : " `id` IN ($ids_str)";
    }

    if ($limit == 0) $limit = "";

    // Assuming DB_fet_rd retrieves data as expected
    $steps = DB_fet_rd("*", "`#_step`", $where, "`catasort` ASC, `id` DESC", $limit);
...
function getValidKrakenInstance()
{
    ...
    try {
        ...
        $json_data = !empty($sql_se['api_kraken']) ? $sql_se['api_kraken'] : []; // Ensure valid JSON data
    ...
    try {
        ...
        foreach ($files as $file) { // Duyệt qua từng tệp và thư mục
            if (is_dir($file)) { // Nếu là thư mục
                // Gọi đệ quy để lấy hình ảnh từ thư mục con
                $images = array_merge($images, getImagesFromDirectory($file));
            } else { // Nếu là tệp
                if (preg_match('/\.(jpg|jpeg|png|gif)$/i', $file)) { // Kiểm tra xem tệp có phải là hình ảnh không (theo đuôi tệp)
                    // Giữ nguyên toàn bộ đường dẫn thư mục và tệp hình ảnh, bắt đầu từ thư mục gốc
                    $relativePath = removeSubstringBefore($file, 'datafiles/');
                    $images[] = $relativePath; // Thêm tệp vào mảng hình ảnh
                }
            }
        }
    } catch (Exception $e) {
...
function update_db_optimized_img($localDirectory)
{
    ...
    try {
        ...
        error_log("Error in update_db_optimized_img: " . $e->getMessage());
    }
}
...
{
    try {
        if (!($kraken instanceof Kraken)) {
            throw new Exception('Invalid Kraken instance.', 400);
        }
        ...
        global $webDirectory;
        $imageName = str_replace('datafiles/', '', $imagePath); // Xóa 'datafiles/' khỏi đường dẫn hình ảnh
        $Url = $_SERVER['DOCUMENT_ROOT'] . '/' . $_SESSION['thumuc']; // Đường dẫn lưu trữ hình ảnh đã tối ưu
        $imageUrl = $webDirectory . '/' . $imageName; // URL của hình ảnh cần tối ưu
        if ($_SESSION['localhost_check']) {
            $imageUrl = 'https://st.quantrimang.com/photos/image/2019/10/30/hinh-nen-phong-canh-dep-1.jpg';
            $imageName = 'test.jpg';
        }
        // Tạo tham số cho Kraken
        $params = [
            "url" => $imageUrl,
            "wait" => true
        ];
        ...
        if (!$data['success']) {
            ...
            throw new Exception($error_message, 405); // Ném lỗi nếu yêu cầu không thành công
        }
        $optimizedImageUrl = $data['kraked_url']; // URL của hình ảnh đã tối ưu
        $optimizedImageContent = file_get_contents($optimizedImageUrl); // Lấy nội dung hình ảnh đã tối ưu
    ...
    try {
        if (!is_dir($logDir)) {
            if (!mkdir($logDir, 0777, true)) {
                throw new Exception("Không thể tạo thư mục log: $logDir");
            }
        }
        if (file_put_contents($logFile, $jsonLogEntry, FILE_APPEND) === false) {
            throw new Exception("Không thể ghi vào file log: $logFile");
        }
        sendMessageToTelegram($jsonLogEntry);
        return true;

    } catch (Exception $e) {
        error_log($e->getMessage(), 3, 'datafiles/logs/error.log'); // Ghi lỗi ra file error.log
        return false;
    }
}
...
function sendMessageToTelegram($message)
{
    try {
        ...
        if (!$telegramApiToken || !$chatId) {
            throw new Exception("Thiếu thông tin token hoặc chat ID.");
        }
        ...
        if (curl_errno($ch)) {
            ...
            throw new Exception("Lỗi khi gửi yêu cầu CURL: $error_msg");
        }
        ...
        if ($httpCode == 200) {
            ...
            throw new Exception("Lỗi từ Telegram API: HTTP $httpCode");
        }

    } catch (Exception $e) {
        // Ghi lỗi vào log
        error_log($e->getMessage(), 3, 'datafiles/logs/error.log');
